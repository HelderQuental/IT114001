...
public class ClientUI extends JFrame implements Event {
    ...
    JPanel textArea;
    JPanel userPanel;
    List<client.User> users = new ArrayList<client.User>();
    private final static Logger log = Logger.getLogger(ClientUI.class.getName());
    Dimension windowSize = Toolkit.getDefaultToolkit().getScreenSize();
    client.GamePanel game;
    String username;
    client.RoomsPanel roomsPanel;
    JMenuBar menu;
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (name != null && name.length() > 0) {
                    ...
                    client.SocketClient.INSTANCE.setUsername(self.username);
        ...
        button.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (text.getText().length() > 0) {
                    client.SocketClient.INSTANCE.sendMessage(text.getText());
                    text.setText("");
                }
            }
    ...
    void createRoomsPanel() {
        roomsPanel = new client.RoomsPanel(this);
        this.add(roomsPanel, "rooms");
    }

    void addClient(String name) {
        client.User u = new client.User(name);
   ...
   // }
    ...
    void goToPanel(String panel) {
        switch (panel) {
            case "rooms":
                ...
                client.SocketClient.INSTANCE.sendGetRooms(null);
                break;
            default:
                // no need to react
                break;
    ...
    void connect(String host, String port) throws IOException {
        client.SocketClient.INSTANCE.registerCallbackListener(this);
        client.SocketClient.INSTANCE.connectAndStart(host, port);
    }
    ...
    public void onClientDisconnect(String clientName, String message) {
        log.log(Level.INFO, String.format("%s: %s", clientName, message));
        Iterator<client.User> iter = users.iterator();
        while (iter.hasNext()) {
            client.User u = iter.next();
